...
public class GMapActivity extends FragmentActivity implements
	...
	public void initPopupWindow() {
		...
		item_images.add(addItems(new Integer[] { R.drawable.pop_normalmap,
				R.drawable.pop_earth, R.drawable.pop_terrin,
				R.drawable.pop_traffic }));
		...
		// 创建弹出菜单对象
		//myMenu = new MyDefinedMenu(this, titles, item_names, item_images,
	//			new ItemClickEventGmap());
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (0 == myMenu.currentState && myMenu.isShowing()) {
			...
			myMenu.showAtLocation(findViewById(R.id.mapGoogle), Gravity.BOTTOM,
					0, 0);
	...
	public void setRecRoute() {
		...
		if (bundle != null) {
			...
			if (!it.hasNext()) {
				Toast toast = Toast.makeText(this, "当前记录无轨迹！",
						Toast.LENGTH_SHORT);
				toast.show();
				return;
			}
			...
			mMap.addMarker(new MarkerOptions().position(
					new LatLng(it.next().getLat(), it.next().getLng())).title(
					"开始"));
			...
			mMap.addMarker(new MarkerOptions().position(
					new LatLng(data.getLat(), data.getLng())).title("结束"));

			mMutablePolyline = mMap.addPolyline(RecOptions.color(Color.GREEN)
					.width(2).geodesic(true));
		}
	}
	...
	private void drawRouteOnMap() {
		if (points.size() >= 2) {
			...
			mMutablePolyline = mMap.addPolyline(options.color(Color.RED)
					.width(2).geodesic(true));
		}
	}
	...
	public void onLocationChanged(Location location) {
		...
		if (status) {
			...
			if (flag == 0) {
				mMap.addMarker(new MarkerOptions().position(
						new LatLng(latitude, longitude)).title("开始"));
				flag = 1;
			}
			...
			if (flag == 1) {
				mMap.addMarker(new MarkerOptions().position(
						new LatLng(latitude, longitude)).title("结束"));
				flag = 2;
			}
		}
